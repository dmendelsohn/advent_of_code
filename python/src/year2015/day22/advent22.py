...
def costToWin(bossHp, usHp, mana, shieldCount, poisonCount, rechargeCount, spell, hardMode=False):
    inputs = (bossHp, usHp, mana, shieldCount, poisonCount, rechargeCount, spell["Name"], hardMode)
    ...
    mana, bossHp, shieldCount, poisonCount, rechargeCount = applyEffects(
    ...
    options = [
        costToWin(bossHp, usHp, mana, shieldCount, poisonCount, rechargeCount, nextSpell, hardMode)
        for nextSpell in SPELLS
    ]
...
def part1Answer():
    options = [costToWin(BOSS_HP, US_HP, US_MANA, 0, 0, 0, spell, False) for spell in SPELLS]
    return min(options)


def part2Answer():
    options = [costToWin(BOSS_HP, US_HP, US_MANA, 0, 0, 0, spell, True) for spell in SPELLS]
    return min(options)
