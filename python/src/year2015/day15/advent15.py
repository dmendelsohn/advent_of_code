...
def computeBestScore(ingredDict, countDictSoFar, numTeaspoonsLeft, exactCalories=None):
    ...
    if len(ingredDict) == len(countDictSoFar):
        ...
        if len(ingreds) == 1:  # Only one ingredient left
            ...
            return computeBestScore(ingredDict, countDictSoFar, 0, exactCalories)
            ...
            for i in range(numTeaspoonsLeft + 1):
                ...
                options.append(
                    computeBestScore(
                        ingredDict, countDictSoFar, numTeaspoonsLeft - i, exactCalories
                    )
                )
            return max(options)
