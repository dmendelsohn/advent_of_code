...
def part1Answer(grid):
    def is_viable(tup):  # Can a go into b?  Each is (size, used)
        a, b = tup
        a_val, b_val = grid[a], grid[b]
        return 0 < a_val[1] <= (b_val[0] - b_val[1])

    return len(list(filter(is_viable, itertools.permutations(grid.keys(), 2))))
...
def part2Answer(grid):
    ...
    fewest += (
        X_LEN - 1 - GAP_X
    )  # Move right to get empty to top right corner (and gets G to (X_LEN,0))
