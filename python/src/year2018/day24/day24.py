...
class Group:
    ...
    def receive_damage(
        self, effective_power, attack_type
    ):  # Updates unit count and returns whether this group is destroyed
...
def parse_line(line):
    ...
    if start_index != -1:
        ...
        descriptions = (
            descriptions.replace(",", "").replace("; ", ";").split(";")
        )  # Commas aren't useful, remove them, and extra space after semicolon
...
def select_targets(armies):  # Return tupe of list of target indices
    ...
    for i in range(2):
        ...
        selection_order = sorted(
            armies[i], key=lambda g: (g.effective_power, g.initiative), reverse=True
        )
        for group in selection_order:
            candidate_targets = filter(
                lambda g: g.index not in targets[i] and g.units > 0, armies[1 - i]
            )
            ...
            target = max(
                candidate_targets,
                key=lambda t: (
                    t.calculate_received_damage(group.effective_power, group.attack_type),
                    t.effective_power,
                    t.initiative,
                ),
            )
